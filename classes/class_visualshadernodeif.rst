...
First two ports are scalar floatin-point numbers to compare, third is tolerance comparsion amount and last three ports represents a vectors returned if ``a == b``, ``a > b`` and ``a < b`` respectivly.
